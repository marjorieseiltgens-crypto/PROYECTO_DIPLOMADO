...
function showView(id){ $$('.view').forEach(v=>v.classList.toggle('active',v.id===id)); $$('.nav-item').forEach(n=>n.classList.toggle('active',n.dataset.view===id)); const labels={inicio:'Resumen ejecutivo',barbecho:'Barbecho químico',trazabilidad:'Trazabilidad de residuos',bioseguridad:'Bioseguridad en terreno',circularidad:'Costos y circularidad',retc:'REPORTE CERTIFICACIONES',datos:'Datos y gobernanza'}; $('#pageTitle').textContent=labels[id]; $$('[data-action="new-record"]').forEach(button=>button.classList.toggle('hidden',id==='trazabilidad')); $('.sidebar').classList.remove('open'); if(id==='trazabilidad')renderRecords();if(id==='inicio')requestAnimationFrame(renderChart); }

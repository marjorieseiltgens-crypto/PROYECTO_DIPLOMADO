...
function traceabilityRows(){
  ...
  return [...(Array.isArray(state.usageLogs)?state.usageLogs:[])].sort((a,b)=>b.date.localeCompare(a.date)).map(log=>{const product=TRACE_PRODUCTS[log.productId]||{group:'Otro',name:log.productId};const cost=Number(costs[log.productId]||0);return {...log,productName:product.name,group:product.group,loss:Number(log.discarded||0)*cost,recovered:Number(log.recycled||0)*cost}}).filter(row=>(group==='all'||row.group===group)&&(!query||`${row.productName} ${row.operator||''} ${row.date}`.toLocaleLowerCase('es').includes(query)));
}
function renderRecords(){
  const rows=traceabilityRows(),totals=rows.reduce((sum,row)=>({used:sum.used+Number(row.used||0),recycled:sum.recycled+Number(row.recycled||0),discarded:sum.discarded+Number(row.discarded||0),loss:sum.loss+row.loss,recovered:sum.recovered+row.recovered}),{used:0,recycled:0,discarded:0,loss:0,recovered:0});
  const formatClp=value=>new Intl.NumberFormat('es-CL',{style:'currency',currency:'CLP',maximumFractionDigits:0}).format(value||0);
  if($('#traceabilityKpis'))$('#traceabilityKpis').innerHTML=[['Registros',rows.length,'Datos desde Costos y circularidad'],['Unidades gestionadas',totals.used+totals.recycled+totals.discarded,`${totals.recycled} recicladas · ${totals.discarded} desechadas`],['Pérdida por descarte',formatClp(totals.loss),'Calculada con costos unitarios'],['Valor recuperado',formatClp(totals.recovered),'Por unidades recicladas']].map(([label,value,detail])=>`<article class="kpi"><div class="kpi-value">${value}</div><div class="kpi-label">${label}</div><div class="kpi-trend">${detail}</div></article>`).join('');
  $('#recordsBody').innerHTML=rows.map(row=>`<tr><td>${fmtDate(row.date+'T12:00')}</td><td><strong>${escapeHtml(row.productName)}</strong><small>${escapeHtml(row.group)}</small></td><td>${Number(row.used||0)}</td><td>${Number(row.recycled||0)}</td><td>${Number(row.discarded||0)}</td><td class="money-negative">${formatClp(row.loss)}</td><td class="money-positive">${formatClp(row.recovered)}</td><td>${escapeHtml(row.operator||'Sin responsable')}</td></tr>`).join('')||'<tr><td colspan="8">No hay registros de Costos y circularidad para este filtro.</td></tr>';
}
...
$('#exportCsv').addEventListener('click',()=>{const rows=traceabilityRows(),header=['Fecha','Grupo','Producto','Utilizadas','Recicladas','Desechadas','Perdida CLP','Recuperado CLP','Responsable'],csv=[header.join(','),...rows.map(r=>[r.date,r.group,r.productName,r.used,r.recycled,r.discarded,r.loss,r.recovered,r.operator].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','))].join('\n');download('resumen_trazabilidad_circular.csv','\ufeff'+csv,'text/csv;charset=utf-8');toast('Resumen de trazabilidad exportado.');});

...
function sanitizeStateData(data){if(Array.isArray(data?.records))data.records=data.records.filter(record=>!/^cart(?:o|\u00f3)n$/i.test(String(record.waste||'').trim()));return data}
function loadState(){ try { return sanitizeStateData(JSON.parse(localStorage.getItem(STORAGE_KEY)) || cloneDemo()); } catch { return cloneDemo(); } }
function saveState(){ state=sanitizeStateData(state);localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); renderAll(); }
...
const TRACE_PRODUCTS={tray288:{group:'Bandejas',name:'Bandeja de 288 alvéolos'},tray144:{group:'Bandejas',name:'Bandeja de 144 alvéolos'},tray72:{group:'Bandejas',name:'Bandeja de 72 alvéolos'},pot05:{group:'Macetas',name:'Maceta de 0,5 litros'},pot06:{group:'Macetas',name:'Maceta de 0,6 litros'},pot075:{group:'Macetas',name:'Maceta de 0,750 cc'},pot1:{group:'Macetas',name:'Maceta de 1 litro'},pot15:{group:'Macetas',name:'Maceta de 1,5 litros'},pot2:{group:'Macetas',name:'Maceta de 2 litros'},pot25:{group:'Macetas',name:'Maceta de 25 litros'}};
function traceabilityRows(){
  const query=($('#recordSearch')?.value||'').trim().toLocaleLowerCase('es'),group=$('#recordTypeFilter')?.value||'all',costs=state.productCosts||{};
  return [...(Array.isArray(state.usageLogs)?state.usageLogs:[])].sort((a,b)=>b.date.localeCompare(a.date)).map(log=>{const product=TRACE_PRODUCTS[log.productId]||{group:'Otro',name:log.productId};const cost=Number(costs[log.productId]||0);return {...log,productName:product.name,group:product.group,discardedKg:Number(log.discardedKg||0),loss:Number(log.discarded||0)*cost,recovered:Number(log.recycled||0)*cost}}).filter(row=>(group==='all'||row.group===group)&&(!query||`${row.productName} ${row.operator||''} ${row.date}`.toLocaleLowerCase('es').includes(query)));
}
function renderRecords(){
  const rows=traceabilityRows(),totals=rows.reduce((sum,row)=>({used:sum.used+Number(row.used||0),recycled:sum.recycled+Number(row.recycled||0),discarded:sum.discarded+Number(row.discarded||0),discardedKg:sum.discardedKg+row.discardedKg,loss:sum.loss+row.loss,recovered:sum.recovered+row.recovered}),{used:0,recycled:0,discarded:0,discardedKg:0,loss:0,recovered:0});
  const formatClp=value=>new Intl.NumberFormat('es-CL',{style:'currency',currency:'CLP',maximumFractionDigits:0}).format(value||0);
  const formatKg=value=>new Intl.NumberFormat('es-CL',{maximumFractionDigits:3}).format(value||0)+' kg';
  if($('#traceabilityKpis'))$('#traceabilityKpis').innerHTML=[['Registros',rows.length,'Datos desde Costos y circularidad'],['Unidades gestionadas',totals.used+totals.recycled+totals.discarded,`${totals.recycled} recicladas · ${totals.discarded} desechadas`],['Pérdida por descarte',formatClp(totals.loss),`${formatKg(totals.discardedKg)} eliminados`],['Valor recuperado',formatClp(totals.recovered),'Por unidades recicladas']].map(([label,value,detail])=>`<article class="kpi"><div class="kpi-value">${value}</div><div class="kpi-label">${label}</div><div class="kpi-trend">${detail}</div></article>`).join('');
  $('#recordsBody').innerHTML=rows.map(row=>`<tr><td>${fmtDate(row.date+'T12:00')}</td><td><strong>${escapeHtml(row.productName)}</strong><small>${escapeHtml(row.group)}</small></td><td>${Number(row.used||0)}</td><td>${Number(row.recycled||0)}</td><td>${Number(row.discarded||0)}</td><td>${formatKg(row.discardedKg)}</td><td class="money-negative">${formatClp(row.loss)}</td><td class="money-positive">${formatClp(row.recovered)}</td><td>${escapeHtml(row.operator||'Sin responsable')}</td></tr>`).join('')||'<tr><td colspan="9">No hay registros de Costos y circularidad para este filtro.</td></tr>';
}
...
function showView(id){ $$('.view').forEach(v=>v.classList.toggle('active',v.id===id)); $$('.nav-item').forEach(n=>n.classList.toggle('active',n.dataset.view===id)); const labels={inicio:'Resumen ejecutivo',barbecho:'Barbecho químico',trazabilidad:'Trazabilidad de residuos',bioseguridad:'Bioseguridad en terreno',circularidad:'Costos y circularidad',perdidas:'Pérdidas económicas',retc:'REPORTE CERTIFICACIONES',emisiones:'Emisiones RETC',datos:'Datos y gobernanza'}; $('#pageTitle').textContent=labels[id]; $$('[data-action="new-record"]').forEach(button=>button.classList.toggle('hidden',id==='trazabilidad'||id==='emisiones'||id==='perdidas')); $('.sidebar').classList.remove('open'); if(id==='trazabilidad')renderRecords();if(id==='emisiones'&&typeof renderEmissions==='function')renderEmissions();if(id==='perdidas'&&typeof renderLossAnalysis==='function')renderLossAnalysis();if(id==='inicio')requestAnimationFrame(renderChart); }
...
$('#exportCsv').addEventListener('click',()=>{const rows=traceabilityRows(),header=['Fecha','Grupo','Producto','Utilizadas','Recicladas','Desechadas','Kg eliminados','Perdida CLP','Recuperado CLP','Responsable'],csv=[header.join(','),...rows.map(r=>[r.date,r.group,r.productName,r.used,r.recycled,r.discarded,r.discardedKg,r.loss,r.recovered,r.operator].map(v=>`"${String(v??'').replaceAll('"','""')}"`).join(','))].join('\n');download('resumen_trazabilidad_circular.csv','\ufeff'+csv,'text/csv;charset=utf-8');toast('Resumen de trazabilidad exportado.');});
...
$('#backupData').addEventListener('click',()=>download('synergiabio_respaldo.json',JSON.stringify(state,null,2))); $('#restoreData').addEventListener('change',async e=>{try{const obj=JSON.parse(await e.target.files[0].text());if(!Array.isArray(obj.records)||!Array.isArray(obj.checklists))throw new Error();state=sanitizeStateData(obj);saveState();toast('Respaldo restaurado.');}catch{toast('El archivo no es un respaldo válido.');}}); $('#resetData').addEventListener('click',()=>{if(confirm('¿Restablecer todos los datos a la demostración inicial?')){state=cloneDemo();saveState();toast('Demostración restablecida.');}}); $$('[data-action="clear-demo"]').forEach(b=>b.addEventListener('click',()=>{if(confirm('¿Eliminar todos los registros de demostración?')){state={demo:false,records:[],checklists:[],infrastructure:0,approvals:[]};saveState();toast('Datos demo eliminados.');}}));

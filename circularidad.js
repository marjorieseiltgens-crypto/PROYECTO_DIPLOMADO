...
function circularSummary(){
  ...
  const rows=CIRCULAR_PRODUCTS.map(p=>{const m=logs.filter(l=>l.productId===p.id),used=m.reduce((s,l)=>s+l.used,0),recycled=m.reduce((s,l)=>s+l.recycled,0),discarded=m.reduce((s,l)=>s+l.discarded,0),cost=Number(state.productCosts[p.id]||0);return {...p,cost,used,recycled,discarded,loss:discarded*cost,recovered:recycled*cost}});
  const totals=rows.reduce((a,r)=>({used:a.used+r.used,recycled:a.recycled+r.recycled,discarded:a.discarded+r.discarded,loss:a.loss+r.loss,recovered:a.recovered+r.recovered}),{used:0,recycled:0,discarded:0,loss:0,recovered:0});
...
function renderCircularity(){
  ...
  $('#circularBody').innerHTML=rows.map(r=>`<tr><td><strong>${escapeHtml(r.name)}</strong><small>${r.group}</small></td><td>${fmtClp(r.cost)}</td><td>${r.used}</td><td>${r.recycled}</td><td>${r.discarded}</td><td class="money-negative">${fmtClp(r.loss)}</td><td class="money-positive">${fmtClp(r.recovered)}</td></tr>`).join('');
  $('#circularFoot').innerHTML=`<tr><td colspan="2">TOTAL DEL PERÍODO</td><td>${totals.used}</td><td>${totals.recycled}</td><td>${totals.discarded}</td><td class="money-negative">${fmtClp(totals.loss)}</td><td class="money-positive">${fmtClp(totals.recovered)}</td></tr>`;
  $('#usageLogsBody').innerHTML=[...state.usageLogs].sort((a,b)=>b.date.localeCompare(a.date)).slice(0,15).map(l=>{const p=CIRCULAR_PRODUCTS.find(p=>p.id===l.productId);return `<tr><td>${fmtDate(l.date+'T12:00')}</td><td>${escapeHtml(p?.name||l.productId)}</td><td>${l.used}</td><td>${l.recycled}</td><td>${l.discarded}</td><td>${escapeHtml(l.operator)}</td></tr>`}).join('')||'<tr><td colspan="6">Aún no hay registros diarios.</td></tr>';
}
...
$('#usageForm').addEventListener('submit',e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.target)),used=Number(d.used),recycled=Number(d.recycled),discarded=Number(d.discarded);if(used+recycled+discarded===0)return toast('Registre al menos una unidad.');ensureCircularState();state.usageLogs.push({...d,id:Date.now(),used,recycled,discarded});state.demo=false;saveState();renderCircularity();$('#usageDialog').close();toast('Movimiento diario registrado.')});
...
$('#exportCircularCsv').addEventListener('click',()=>{const {rows}=circularSummary(),header='Producto,Costo unitario,Utilizadas,Recicladas,Desechadas,Perdida CLP,Recuperado CLP',csv=[header,...rows.map(r=>[r.name,r.cost,r.used,r.recycled,r.discarded,r.loss,r.recovered].map(v=>`"${String(v).replaceAll('"','""')}"`).join(','))].join('\n');download('reporte_circularidad.csv','\ufeff'+csv,'text/csv;charset=utf-8')});
